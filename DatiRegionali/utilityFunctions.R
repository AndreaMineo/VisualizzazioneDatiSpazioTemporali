...
generateDataForSpatialPlot <- function(data,map,variable,date){
  ...
  merged <- merge(df,map,by="region_name")
...
generateDataForTimeSeriesPlot <- function(data,variable,setOfRegions){
  
  if("all" %in% setOfRegions){
    s <- unstack(data[,c(variable,"region_name")])
  }else{
    s <- unstack(data[data$region_name %in% setOfRegions,c(variable,"region_name")])
  }
